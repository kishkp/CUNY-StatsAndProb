...
**The distribution of the sample is right skewed.**
**The 'typical' size in my sample is `r median(samp)`**
**On increasing the breaks in the histogram, we find that there are many outliers that might influence the mean. Hence a median would provide a better measure of central tendancy. 'typical' in my interpretation is a paramater using which we can infer about the population**


2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

**I would expect another student's distribution to be similar but not the same (if that is what is meant by identical) as mine. A truely random sample would yield a distribution that mimics the population. Hence, in most cases, we would end up having a similar distribution**
...
**Yes, the population mean is captured by my confidence internal.**
**In about 95% of the samples, the true mean is captured by the confidence interval of the respective sample.** 
...
**In my run, I found that 2 samples' CI did not contain the population mean. This works out to `r (1 - (2/50)) * 100`%. This is not equal to the confidence level. The confidence interval only provides a  plausible range of values for a parameter. It does not rule out other values based on the data.**   

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
**Assuming a 99% confidence interval, the appropriate critical values would be `(sample_mean +/- 2.58 * se)`**
