...
source("https://raw.githubusercontent.com/kishkp/CUNY-StatsAndProb/master/Lab1/more/cdc.R")
```
...
**There seems to be a good correlation between the weight and desired weight.**
...
**wdiff is continuous. If wdiff is 0 it means that the person is of ideal weight. If wdiff is positive then it means that person feels underweight. If it is negative, then it means that the person feels overweight.**
...
**The histogram  gives us an indication of the shape of the distribution. From the histogram it seems that the distribution is slightly left skewed.**

**The mean is approximately -15 and median is -10. Since the mean is to the left of the median, it shows the left skew in the data.** 

**The shape is unimodal with the mode at slightly below zero.**

**Overall the wdiff indicates that slightly more people view themselves as overweight.**  
...
summary(cdc[cdc$gender%in% c('m'),]$wdiff)
summary(cdc[cdc$gender%in% c('f'),]$wdiff)

boxplot(cdc$wdiff~cdc$gender, outline=FALSE)

```

**From the respective summaries and boxplot it seems that women are more sensitive to their weights than men but only slightly.**  
...
nrow(cdc[cdc$weight>(Mean-SD) & cdc$weight<(Mean+SD),]) /  nrow(cdc)

```

**70.76% of the weights are within 1 standard deviation of the mean.**
