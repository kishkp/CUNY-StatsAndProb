...
**The Histogram of the streak lengths indicate that this is a right skewed distribution. The typical streak length is 0 as indicated by the median. The longest streak is 3**
...
**Since the shots are independent of each other, the distribution would be somewhat similar. The probability of it being exacly the same is vey less. Since we are taking samples, the samples may not be the same and hence the variations between samples**
...
***Both the distributions are right skewed with a median of 0 and similar IQR. However, the max streak lengths of both differ. If we were to consider all the data then the distributions would not match exactly***     
